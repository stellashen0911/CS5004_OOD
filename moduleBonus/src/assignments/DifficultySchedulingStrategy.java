 ...
 * It sorts the given assignments in Difficulty lexicographical order.
 */
public class DifficultySchedulingStrategy implements SchedulingStrategy {
  
  /** Default constructor. */
  public DifficultySchedulingStrategy() {}
  ...
  public String schedule(AssignmentList listOfAssignment) throws IllegalArgumentException {
    ...
    listOfAssignment.setOrdering("difficulty");
    ...
    for (int i = start + 1; i <= end; i++) {
      ...
      int j = i - 1;
      while ((j >= start) 
          && (current.getDifficulty() > currList.get(j).getDifficulty())) {
